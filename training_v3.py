...
def initialize_variables(pruning_number):
    with open('weights_log/weights'+str(pruning_number)+'.pkl','rb') as f:
...
def prune_weights(pruning_number, threshold, weights, weight_mask, biases, biases_mask):
    percent = (pruning_number + 1) * 10
    keys = ['cov1','cov2','fc1','fc2']
    next_threshold = {}
    b_threshold = {}
    for key in keys:
        weight = weights[key].eval()
        bias = biases[key].eval()
        next_threshold[key] = np.percentile(np.abs(weight),percent)
    ...
    print('threshold used for {}, is : {}'.format(pruning_number+1, next_threshold))
...
def main(argv = None):
    ...
    try:
        ...
        if (pruning_number == 0 or TRAIN == False):
            weights_mask = {
        ...
        (weights, biases) = initialize_variables(pruning_number)
        # Construct model
        ...
        with tf.Session() as sess:
            ...
            if (TRAIN == False):
                print("Disabled training, pruning count is {}".format(pruning_number))
